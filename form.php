...
$conn   = new PDO ("pgsql:host={$host};port={$port};dbname={$dbname};user={$user};");
...
function q($post){
    ...
    function valid(array $post): array
    {
        ...
        if (!empty($post['name']) and !empty($post['email']) and !empty($post['age'])) {
            ...
            $validateForm = validNameEmailAge($name, $email, $age, $constraints);
    ...
    function validNameEmailAge(string $name, string $email, string $age, array $constraints): array
    {
